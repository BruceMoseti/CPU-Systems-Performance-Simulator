...
[![build and test](https://github.com/BruceMoseti/CPU-Systems-Performance-Simulator/actions/workflows/ci.yml/badge.svg)](https://github.com/BruceMoseti/CPU-Systems-Performance-Simulator/actions/workflows/ci.yml)

A configurable model of a CPU, its caches and its main memory, together with the
tooling to run architecture experiments against it. It answers one question:
...
`random_access` spends its cycles waiting for a free miss slot, so supplying more
of them fixes it: quadrupling the MSHRs is worth **+176%**. `pointer_chase` waits
on DRAM itself, and that same change is worth **+0.0%**, because every load's
address is the previous load's result and nothing can overlap. Hit rates cannot
tell these two apart. Overlap can, and that is what this model is built around.

So the tool does not stop at naming the bottleneck — it runs each candidate
change and reports the measured effect:
...
Ten parameter sweeps built on that (372 simulations, ~18 s) produce the
[findings](#findings) below, and the model is [checked against the machine it
runs on](#model-against-hardware), where it reproduces a dependent-load chain to
within measurement noise and visibly fails elsewhere.
...
cmake --build build -j
ctest --test-dir build            # 39 unit tests
...
Cycles are accumulated in integers throughout, including the fractional cycle a
wide machine carries between instructions, which is held in fixed point. A
performance model that sums cycles in floating point is quietly sensitive to
compiler flags and FMA contraction; integer accounting makes the numbers
reproducible by construction. It costs about 1.5% throughput against a `double`
accumulator, measured, and is worth it.
...
`results/`, which come from 372 simulations across ten sweeps.
...
The host used for the measurements below exposes no performance counters
(`perf_event_open` returns ENOENT for every hardware event), which is the normal
situation inside a virtual machine, so validation relies on two things that need
no PMU: the host's own cache geometry from sysfs, and its real access latency
measured by timing a dependent pointer chase over growing working sets.
...
Native times are the fastest of nine runs; the spread column is the range across
them, because a ratio is only meaningful to the precision of the measurement
underneath it. Full output is in
[results/validation.json](results/validation.json).

| workload | native | spread | model | model/native |
| --- | --- | --- | --- | --- |
| `pointer_chase` | 25.90 ms | 7.7% | 27.07 ms | **1.05×** |
| `matrix_naive` | 5.72 ms | 0.6% | 3.71 ms | 0.65× |
| `strided` | 2.51 ms | 3.6% | 0.73 ms | 0.29× |
| `random_access` | 2.82 ms | 4.7% | 1.66 ms | 0.59× |
| `matrix_blocked` | 2.05 ms | 8.0% | 3.74 ms | 1.82× |
| `sequential` | 0.32 ms | 17.9% | 0.42 ms | 1.30× |

### What the model captures

**A dependent load chain, to within measurement noise.** `pointer_chase` is the
case the model describes directly — one exposed miss at a time — and it predicts
a 25.90 ms measured runtime as 27.07 ms, a 5% difference against a 7.7%
measurement spread. The measured latency curve is also reproduced by the model's
own hierarchy, which is a consistency check on the cache geometry and on the
...
The `sequential` row is the one to be careful with: at 0.32 ms with a 17.9%
spread it is too short to time reliably on a shared machine, so its 1.30× ratio
is not far outside the noise and no conclusion is drawn from it alone.
...
`matrix_naive` (0.65×), `matrix_blocked` (1.82× the other way). All three sit
well outside their measurement spread, so these are real modelling errors rather
than noise. The sharpest form of it:
...
measured `random_access`/`sequential` is 8.83×, the model says 3.97×. This is
...
simulator. With no `perf` binary on the measurement host, the simulator was
profiled with gprof (`-DPERFSIM_PROFILE=ON`). Replaying a 21.3M-record trace
showed three hotspots; full before/after profiles are in
...
End-to-end throughput over all six traces (51M records). Both revisions were
built and their runs interleaved, so that drift in machine speed over the
measurement affects the two equally — measuring them sequentially understated the
gain by four points:
...
**1.66× throughput, 40% less wall time.** `benchmark.py` also reports peak
resident memory, which is 5 MB for the baseline machine and 23 MB when modelling
a 64 MB L2.

Correctness was the constraint, and the unit tests were the guard: all passed
...
tests/                39 unit tests, each with hand-computed expectations
...
Cache logic and timing accounting are easy to get subtly wrong, and wrong in a
way that still produces plausible-looking numbers. What guards against that:

- **39 unit tests**, each with a hand-computed expectation rather than a golden
  value recorded from the code. Cold, capacity and conflict misses; LRU choosing
  the correct victim rather than the oldest insertion; the exact cycle count when
  one MSHR serialises four misses (785 cycles); bandwidth saturation being
  reported as bandwidth rather than as MSHR pressure.
- **Address and undefined-behaviour sanitizers** over the tests and over every
  binary across all six workloads and five configurations. This found a real bug:
  an out-of-range value in a config file reached a double-to-integer conversion,
  which is undefined, so the range is now checked while the value is still a
  double.
- **~1,100 malformed inputs** pushed through both hand-rolled parsers under those
  sanitizers, since the trace reader parses with raw pointers. Truncations of
  valid input at every offset, random bytes, and adversarial operands.
- **Byte-for-byte reproducible results.** Two runs of the full sweep produce
  identical CSVs, so a re-run diffs to nothing unless the model changed, and GCC
  and clang produce identical cycle counts. This is also how the optimisation
  work was verified: 30 workload/config combinations compared field by field
